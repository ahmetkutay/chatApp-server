...
async function checkExistingUserForLogin(userData) {
  ...
  if (findUserEmail || findUsername || findUserMobileNumber) {
    if (findUserEmail) {
      Logger.info(
        `POST /api/auth/login logged by email and user - email: ${findUserEmail.email} !`
      );
      return findUserEmail;
    } else if (findUsername) {
      Logger.info(
        `POST /api/auth/login logged by username and user - username: ${findUsername.username}!`
      );
      return findUsername;
    } else if (findUserMobileNumber) {
      Logger.info(
        `POST /api/auth/login logged by mobileNumber and user - mobileNumber: ${findUserMobileNumber.mobileNumber}!`
      );
